...
def test(model_path, test_data, reversed_dictionary):
	...
	num_acc_batches = 30
        check_batch_idx = np.random.randint(num_acc_batches)
        acc_check_thresh = 5
        accuracy = 0
        for batch in range(num_acc_batches):
            if batch == check_batch_idx:
                
		true_vals, pred, current_sstate, acc,cost = sess.run([m.input_obj.targets, m.predict, m.state, m.accuracy,m.cost],
                                                               feed_dict={m.init_state: current_state}) 
         
                pred_string = [reversed_dictionary[x] for x in pred[:20]]
		print("\n$$$$$$$$$$$$$$$$$$$$$$ENTENCEEEEEEEEEEEEEEEEEEEEEEEEEEEEE\n")
...
#          model_save_name='monish')
trained_model = data_path + "monish-final"
